...
#            relative_entity_name = 'world'
   ...
   # https://answers.gazebosim.org//question/18372/getting-model-state-via-rospy/            
    ...
    def step(self, action, episode_steps):        
        ...
        # DRL-VO Implication, 
        # 1. State 받아오기
        state_DRLVO = self._get_observation()   # 2 x 80 x 80 = 12800
        #state_DRLVO
        
        # state = laser_state(20) + robot_state(4) + human_state(12800)
        state_DRLVO_new = np.append(state, state_DRLVO)
        ...
        return state_DRLVO_new, reward_DRLVO, done, target
...
#        print('페드포스:',self.ped_pos, '스캔:',self.scan, '골:',self.goal)
    ...
    def get_reward_path_230206_VO(target, collision, action, goal_x, goal_y, skew_x, skew_y, pre_dist, dist, laser_state, mht_peds):
        ...
        R_smooth = 0.0
        if abs(action[1]) > 1.0:
            R_smooth = -0.1 * abs(action[1])
        # total
        R_t = R_g + R_p + R_c + R_th + R_smooth
        return R_t    
