...
class RrtApf:
    ...
    def __init__(self, start, goal, obs_map, offset=6, maxIter=1000, goal_radius=15, animate=False):
        self.start = start
        self.goal = goal
...
def get_obstacle_map(pts, pedsim_agent_list, r_x, r_y):
    ...
    map_bias = 5.5   # 4.5
    obs = check_in_poly(pts, RES*create_rect(0+map_bias, -3+map_bias, 0.5, 6.0).reshape(-1,2))

    # wall_11
    obs2 = check_in_poly(pts, RES*create_rect(-3.0+map_bias, 0+map_bias, 6.0, 0.5).reshape(-1,2))
    obs = np.logical_or(obs, obs2)
    # wall_s3
    obs3 = check_in_poly(pts, RES*create_rect(3.0+map_bias, 0+map_bias, 6.0, 0.5).reshape(-1,2))
    obs = np.logical_or(obs, obs3)
    
    # 우상단 상자
    # wall_15
    obs4 = check_in_poly(pts, RES*create_rect(-2+map_bias, 3.0+map_bias, 0.5, 2.0).reshape(-1,2))  # height, width
    obs = np.logical_or(obs, obs4)
    
    # wall_16
    obs5 = check_in_poly(pts, RES*create_rect(2+map_bias, 3.0+map_bias, 0.5, 2.0).reshape(-1,2))
